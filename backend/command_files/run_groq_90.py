...
import time
import csv
from ingest.priority import infer_kind_priority 

load_dotenv()

DB_PATH = r"D:\My Docs\Product Management\P4\Working Code\max sophisticated\backend\data\kg_90.sqlite"

IN_PATH = Path(__file__).resolve().parents[1] / "data" / "groq_batch_90.jsonl"
OUT_PATH = Path(__file__).resolve().parents[1] / "data" / "groq_batch_90_outputs.jsonl"
INCOMPLETE_OUT_PATH = OUT_PATH.with_name(OUT_PATH.stem + "groq_batch_90_incomplete.jsonl")
CSV_OUT_PATH = Path(__file__).resolve().parents[1] / "data" / "groq_batch_90_outputs.csv"
...
# Raise this (your 10-url script used 2000)
DEFAULT_MAX_COMPLETION_TOKENS = 2493

# Keep same “structured output” approach as your 10-run.
# Note: Groq docs say json_schema is supported only on supported models; json_object is older JSON mode. [web:1732]
RESPONSE_FORMAT = {
    "type": "json_schema",
    "json_schema": {
        "name": "cognitive_map_b_min",
        "strict": False,
        "schema": {
            "type": "object",
            ...
            "required": ["root", "domains", "themes", "concepts", "micro_insights"],
        },
    },
}
