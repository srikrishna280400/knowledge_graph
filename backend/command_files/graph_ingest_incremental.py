...
def get_pg_mirror_engine() -> Engine:
    db_url = os.getenv("DATABASE_URL") or os.getenv("KG_PRIMARY_DATABASE_URL")
    if not db_url:
        raise RuntimeError("DATABASE_URL or KG_PRIMARY_DATABASE_URL is not set")

    engine = create_engine(db_url, future=True, pool_pre_ping=True)

    if engine.dialect.name != "postgresql":
        raise RuntimeError(f"Expected PostgreSQL mirror, got {engine.dialect.name}")
...
def _sqlite_rows_for_ids(con: sqlite3.Connection, sql_prefix: str, ids: list[str], tail_sql: str = "", extra_params=()) -> list[dict]:
    if not ids:
        return []
...
def mirror_batch_to_postgres(
    ...
    pg_engine = get_pg_mirror_engine()
    ...
    with pg_engine.begin() as pg:
        if source_nodes:
            pg.execute(text("""..."""), source_nodes)

        if concept_nodes:
            pg.execute(text("""..."""), concept_nodes)

        if source_payloads:
            pg.execute(text("""..."""), source_payloads)

        if source_concept_edges:
            pg.execute(text("""..."""), source_concept_edges)

        if source_embeddings:
            pg.execute(text("""..."""), source_embeddings)

        if concept_embeddings:
            pg.execute(text("""..."""), concept_embeddings)

        if concept_edges:
            pg.execute(text("""..."""), concept_edges)

        if source_edges:
            pg.execute(text("""..."""), source_edges)

        if concept_stats:
            pg.execute(text("""..."""), concept_stats)
