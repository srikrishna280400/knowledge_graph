...
from app.make_groq_budget_queue import (
    BUDGET_USD,
    MAX_COMPLETION_TOKENS,
    estimate_cost_usd,
)

from migration.config import PRIMARY, KG_90, make_timestamped_batch_path
from migration.db_factory import new_session, attach_database
...
You must output "connections" as an array of objects with this EXACT structure:
...
connections: [
  {
    ...
    "link_type": "surface" | "deep" | "temporal" | "contradiction" | "synthesis",
    "strength": 0.0-1.0,
    "why": "",
    "shared_signals": [],
    "action_potential": "Someday/Maybe" | "Active Project" | "Reference" | "Inspiration",
...
"connections": [
  {
    "target": {"canonical_url": "", "hint_title": ""},
    "link_type": "surface",
    "strength": 0.0,
    "why": "",
    "shared_signals": [],
    "action_potential": "Someday/Maybe",
    "emotion": "Curiosity"
  }
],
...
OUT_PATH = make_timestamped_batch_path(PRIMARY).with_name(
    make_timestamped_batch_path(PRIMARY).name.replace("groq_batch_all", "groq_batch_x")
)
...
def build_user_content(it: SavedItem) -> str:
    ...
    if it.status == "title_only":
        text = "(Post unavailable/removed; title-only.)"
    else:
        text = (it.extracted_text or "").strip() or "(Empty extracted_text.)"

    payload, _dbg = build_prompt_content(
        title=title,
        extracted_text=text,
        min_chars=1200,
...
def main():
    ...
    session.close()
    print(f"OK: wrote {written} X items to {OUT_PATH}")
