...
REDDIT_CRAWL_LIMIT = 0
LINKEDIN_CRAWL_LIMIT = 0
X_CRAWL_LIMIT = 0
GENERIC_CRAWL_LIMIT = 2

MIRROR_DATABASE_URL = (
    os.getenv("KG_PRIMARY_MIRROR_DATABASE_URL")
...
def get_mirror_sessionmaker():
    ...
    if _MIRROR_SESSIONMAKER is None:
        pg_url = normalize_pg_url(MIRROR_DATABASE_URL)
...
def run_generic_queue(limit: int | None = None):
    ...
    try:
        with mirror_session_scope() as mirror_session, httpx.Client(
...
def main():
    ...
    if reddit_n > 0 and REDDIT_CRAWL_LIMIT > 0:
        run_script("app.reddit_crawl", min(reddit_n, REDDIT_CRAWL_LIMIT))
    if linkedin_n > 0 and LINKEDIN_CRAWL_LIMIT > 0:
        run_script("app.linkedin_crawl", min(linkedin_n, LINKEDIN_CRAWL_LIMIT))
    if x_n > 0 and X_CRAWL_LIMIT > 0:
        run_script("app.x_crawl", min(x_n, X_CRAWL_LIMIT))
    if generic_n > 0 and GENERIC_CRAWL_LIMIT > 0:
        run_generic_queue(min(generic_n, GENERIC_CRAWL_LIMIT))
