...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--ids-file", default="", help="JSON array of SavedItem ids to batch exactly")
    return p.parse_args()
...
def load_ids_file(path_value: str) -> list[str]:
    ...
    if not isinstance(data, list):
        raise ValueError("--ids-file must contain a JSON array")
    ...
    try:
        ...
        if mirror_session is not None:
            try:
                ...
                print(f"⚠️ mirror llm_batched ensure failed; local sqlite write kept: {e}")
        ...
        if selected_ids:
            ...
            print(
                f"[make_batch_all] ids_file mode: requested={len(selected_ids)} "
                f"found={len(items)} missing={len(missing)}"
            )
            print(f"[make_batch_all] selected_ids_sample={selected_ids[:10]}")
            if missing:
            ...
            if llm_processed_exists:
                stmt = stmt.where(
                    text(
                        "NOT EXISTS (SELECT 1 FROM llm_processed p "
                        "WHERE p.saved_item_id = saved_items.id)"
                    )
                )
        ...
        written = 0
        spent_est_usd = 0.0
        enforce_budget = False if selected_ids else (args.limit is None)
            ...
            for it in items:
                ...
                if mirror_session is not None:
                    try:
                        ...
                        mirror_session.rollback()
                        print(f"⚠️ mirror llm_batched sync failed; local sqlite write kept: {e}")

        print(f"OK: wrote {written} items to {out_path}")
        print(f"Estimated cost cap=${BUDGET_USD} spent~${spent_est_usd:.4f}")
    finally:
        session.close()
        if mirror_session is not None:
